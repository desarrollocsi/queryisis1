...
BEGIN
  ...
  for data in 	
    ...
    CASE
	WHEN a.nprfctra is not null THEN 'CONSULTA AMBULATORIA'
	WHEN tipoDeVenta.esTituloGratuito THEN 'CONSUMO - TITULO GRATUITO'
	ELSE 'CONSUMO' 
	END AS cons_amb,
    ...
    from pwccp01 where pwccp01.cprcdmnto = 29 /*CONSULTA AMBULATORIA*/) a ON(a.nprfctra = pwtgn00.nprfctra),
    LATERAL (SELECT b.tvnta LIKE 'T' AS esTituloGratuito FROM pwccp00 b WHERE  b.clcl=1 AND b.cemprsa =1 AND b.nprfctra=pwtgn00.nprfctra) AS tipoDeVenta
